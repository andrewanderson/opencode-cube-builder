...
## 2026-08-30 — Pending adds (confirmed, awaiting cuts)

Cards confirmed for addition this session but not yet in the CubeCobra mainboard — user is handling the cut side. Move to `decisions.md` once physically added.

- **Jarad, Golgari Lich Lord** + **Sidisi, Brood Tyrant** — BG stretch pair (see decisions.md 2026-08-30 for rationale).
- **Duplicant** — Tinker target pool addition; colorless 6cmc removal-on-a-body, deliberately fair (not game-ending) per the Portal to Phyrexia walk-back. Steel Hellkite and Myr Battlesphere were also proposed as Tinker targets but not yet confirmed — still open if more Tinker depth is wanted later.
